...
class BaseSetposCommand(BaseThreadedCommand):
    
    numthreads=4      
            
    def call_each_execute(self, obj, pos, *args, **kwargs):
        self.info("Setting current position of '{0}' to {1} {2}...".format(obj.name, pos, obj.units))
        super(BaseSetposCommand,self).call_each_execute(obj, pos,*args,**kwargs)
